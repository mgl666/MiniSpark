...
| `/load <名称>` | 切换到指定会话 |
| `/delete <名称>` | 删除指定会话的历史 |
| `/history` | 查看当前会话全部消息（完整显示，不截断） |
| `/history -n N` | 查看当前会话最近 N 条消息（完整显示，不截断） |
...
QQ 通道支持后台静默运行模式，不占用终端窗口，适合服务器长期部署。

```bash
# 前台运行（Ctrl+C 停止，占用当前终端）
python -m minispark qq

# 后台静默运行（不占用终端，进程常驻后台）
python -m minispark qq --background
python -m minispark qq -b               # 简写
...
后台运行时，进程信息存储在 `~/.minispark/qq.pid`，运行日志写入 `~/.minispark/qq.log`。重复执行 `--background` 时会自动检测已有进程，避免重复启动。

**运行模式对比：**

| 命令 | 终端占用 | 日志输出 | 停止方式 | 适用场景 |
|---|---|---|---|---|
| `python -m minispark qq` | 是 | 终端实时 | Ctrl+C | 开发调试 |
| `python -m minispark qq -b` | 否 | `~/.minispark/qq.log` | `qq-stop` | 服务器部署 |
...
# 3. 安装依赖（一条命令自动安装所有核心依赖）
pip install -e .
```

核心依赖清单（共 8 个，全部轻量）：`openai`、`httpx`、`websockets`、`certifi`、`pydantic`、`typer`、`rich`、`apscheduler`。

由于使用 `pip install -e .`（可编辑模式）安装，后续更新只需拉取最新代码即可，修改立即生效：

```bash
git pull
```

如果上游新增了依赖，重新执行一次安装（已安装的包不会重复安装）：
